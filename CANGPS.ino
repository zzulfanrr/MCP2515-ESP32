...
void setup() {
  ...
  // Wait for GPS location to be valid
  // Serial.println("Waiting for valid GPS location...");
  // while (true) {  // Wait indefinitely for valid GPS data
  //   if (GPSSerial.available() > 0) {
  //     gps.encode(GPSSerial.read());
  //     if (gps.location.isValid()) {
  //       latitude = gps.location.lat();
  //       longitude = gps.location.lng();
  //       Serial.print("GPS Location obtained: Latitude = ");
  ...
  String csvcol = "#,manifold_air_pressure,engine_rpm,vehicle_speed,iat,latitude,longitude, gpstime";  //
...
void loop() {
  ...
  while (GPSSerial.available() > 0) {
    ...
    // if (gps.time.isValid()) {
    //   gpshour = gps.time.hour() + 7;  // Adjust for timezone
    //   gpsminute = gps.time.minute();
    //   gpssecond = gps.time.second();

    //   gpsTime = String(gpshour) + ":" + ((gpsminute < 10) ? "0" : "") + String(gpsminute) + ":" + ((gpssecond < 10) ? "0" : "") + String(gpssecond);
    // } else {
    //   // Set default values if GPS time is not available
    // }
  }
  ...
  if (allResponsesReceived) {
    ...
    // String csvlog = String(count++) + "," + String(manifold_air_pressure) + "," + String(engine_rpm) + "," + String(vehicle_speed) + "," + String(iat) + "," + String(latitude, 6) + "," + String(longitude, 6) + "," + String(gpsTime);  // + "," + String(gpsTime)
